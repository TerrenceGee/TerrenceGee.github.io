...
- 构建器和静态工厂都没有办法解决很好的扩展大量的参数
- JavaBeans方法，即使用无参构造器先创建对象，再调用setter给属性赋值的方式，虽然可以达到客户端代码可读的目的。但是它不安全，将对象多次进行修改
- 构建器方法非常适合多参数的对象，但是注意，如果是需要扩展参数的对象中，最好一开始就使用建造者，而不是一开始使用构建器，而到后面再改用建造者
- 建造者方法的缺点是：它需要先创建内部静态类对象，这个需要一定的开销，另外它会让客户端代码变得更加冗长（注意冗长而不是晦涩，可读性还是比较好的）
